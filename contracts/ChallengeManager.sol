...
contract ChallengeManager is IChallengeManager {
    ...
    function bisectAssertion(
        ...
        challenge.challengeState = keccak256(abi.encodePacked(
    ...
    function generateBisectionDataImpl(BisectAssertionData memory _data) private pure returns (bytes32, bytes32[] memory) {
        ...
        for (j = 0; j < _data.bisectionCount; j++) {
            ...
            frame.hashes[j] = keccak256(abi.encodePacked(
                frame.preconditionHash,
                ArbProtocol.generateAssertionHash(
                    _data.afterHashAndMessageAndLogsBisections[j],
                    frame.stepCount,
                    _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount + j],
                    _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount + j + 1],
                    _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount * 2 + 1 + j],
                    _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount * 2 + 2 + j],
                    frame.coinAmounts
                )
            ));
            frame.beforeHash = _data.afterHashAndMessageAndLogsBisections[j];
            ...
            if (j == 0) {
                ...
                frame.fullHash = keccak256(abi.encodePacked(
                    frame.preconditionHash,
                    ArbProtocol.generateAssertionHash(
                        _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount - 1],
                        _data.totalSteps,
                        _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount],
                        _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount * 2],
                        _data.afterHashAndMessageAndLogsBisections[_data.bisectionCount * 2 + 1],
                        _data.afterHashAndMessageAndLogsBisections[_data.afterHashAndMessageAndLogsBisections.length - 1],
                        frame.coinAmounts
                    )
                ));
            }
